...
public class CarboardPointerCustom : MonoBehaviour
{
	/// <summary>
	...
	/// </summary>
	private const float _RETICLE_MIN_OUTER_ANGLE = 0.2f;

	/// <summary>
	/// Angle at which to expand the reticle when intersecting with an object (in degrees).
	/// </summary>
	private const float _RETICLE_GROWTH_ANGLE = 0.55f;
	...
	/// </summary>
	[SerializeField] private float _RETICLE_MAX_DISTANCE = 4f;
	...
	/// </summary>
	private const float _RETICLE_GROWTH_SPEED = 4.0f;
	...
	private void Update()
	{
		...
		if (GameObject.Find("UI_IcoHand") is GameObject uiIcoHand)
		{
			uiIcoHand.GetComponent<RawImage>().enabled = (_gazedAtObject != null && IsInteractive(_gazedAtObject));
		}
